...
func TestInjectLabels(t *testing.T) {
	...
	// IP now.
	IPIdentityCache.metadata.upsert(inClusterPrefix, source.CustomResource, "node-uid", labels.LabelRemoteNode)
	assert.Len(t, IPIdentityCache.metadata.m, 2)
	remaining, err = IPIdentityCache.InjectLabels(ctx, []netip.Prefix{inClusterPrefix})
	assert.NoError(t, err)
	assert.Len(t, remaining, 0)
...
func TestFilterMetadataByLabels(t *testing.T) {
	...
	IPIdentityCache.metadata.upsert(netip.MustParsePrefix("2.1.1.1/32"), source.Generated, "gen-uid", labels.LabelWorld)
	IPIdentityCache.metadata.upsert(netip.MustParsePrefix("3.1.1.1/32"), source.Generated, "gen-uid-2", labels.LabelWorld)
...
func TestRemoveLabelsFromIPs(t *testing.T) {
	...
	// called.
	IPIdentityCache.metadata.upsert(worldPrefix, source.CustomResource, "kube-uid", labels.LabelKubeAPIServer)
	remaining, err = IPIdentityCache.InjectLabels(ctx, []netip.Prefix{worldPrefix})
	assert.NoError(t, err)
	assert.Len(t, remaining, 0)
...
func setupTest(t *testing.T) {
	...
	IPIdentityCache.metadata.upsert(worldPrefix, source.CustomResource, "kube-uid", labels.LabelKubeAPIServer)
	IPIdentityCache.metadata.upsert(worldPrefix, source.Local, "host-uid", labels.LabelHost)
}
